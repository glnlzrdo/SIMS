...
public class PurchaseScreenController {
	...
	private void initialize() throws FileNotFoundException {
		...
		if (purchaseList == null)
			transactionNumber.textProperty().setValue("00-000-" + String.format("%04d", 1));
		else
			transactionNumber.textProperty().setValue("00-000-" + String.format("%04d", purchaseList.size() + 1));
			//formattedTransNumber =
			//transactionNumber.setText(Integer.toString();


		//transactionNumber.textProperty().setValue(formattedTransNumber);
	...
	private void searchBoxClicked() {
		quantityTextbox.textProperty().setValue("");
	}
	...
	private void addItem() {
		...
		productsForTable.get(productsForTable.size() - 1)
				.setSubTotal(productsForTable.get(productsForTable.size() - 1).getPriceForPurchase()
	...
	private void paymentClicked() throws FileNotFoundException {
		...
		purchaseList.add(new Purchase(purchaseList.size() + 1, dateLabel.getText(),
		...
		if (purchaseList == null)
			...
			transactionNumber.textProperty().setValue("00-000-" + String.format("%04d", purchaseList.size() + 1));
		DatabaseController.savePurchaseTransactions(purchaseList);
		refreshList();
	}
